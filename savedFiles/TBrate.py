...
# Flavor histogram - JL
Btrue		= TH1F("Btrue",		"% of true b's per flavor",	 	3,   0,    4 )
Btrueh		= TH1F("Btrueh",	"% of true b's per flavor; high pt",	3,   0,    4 )
Btruel		= TH1F("Btruel",	"% of true b's per flavor; low pt",	3,   0,    4 )
		...
		#Now we start top-tagging.  In this file, we use a sideband based on inverting some top-tagging requirements
			...
			if sjbtag_cut:
				...
				event.getByLabel (partonFlavourLabel, partonFlavourHandle)
    				flavour 	= 	partonFlavourHandle.product() 
				...
				if bmass_cut:
					...
					#Probably not the most efficient way with all the other btag_cuts up there in eta region cuts
					if btag_cut:

						# Fill histogram for quark flavors -JL
						# udsg: 1,2,3,(9)21 ; b: 5 ; charm: 4

						# Prepare quarks for binning (1:udsg, 2:charm, 3:b)
						tempFlav = 0
						binName = ""
						if 1<=abs(flavour[bindexval])<4 or abs(flavour[bindexval]) == 21:
							tempFlav = 1
							binName = "other"
						elif abs(flavour[bindexval]) == 4:
							tempFlav = 2
							binName = "charm"
						elif abs(flavour[bindexval]) == 5:
							tempFlav = 3
							binName = "b"
						else:	#error accounting
							print "Anomalous quark"
							print flavour[bindexval]
		
						Btrue.Fill(tempFlav)
		
						# Use when QCD sample is not pre-sorted by pt
						# Split into low/high pt regions
						#if 0.0 < bjet.Perp() < 450.0:
						#	Btruel.Fill(tempFlav)

						#if bjet.Perp() >= 450.0:
						#	Btrueh.Fill(tempFlav)
...
# Normalizing true btag rate histogram

Btrue.GetXaxis().SetBinLabel(1, "other")
Btrue.GetXaxis().SetBinLabel(2, "charm")
Btrue.GetXaxis().SetBinLabel(3, "b")

#Btruel.GetXaxis().SetBinLabel(1, "other")
#Btruel.GetXaxis().SetBinLabel(2, "charm")
#Btruel.GetXaxis().SetBinLabel(3, "b")

#Btrueh.GetXaxis().SetBinLabel(1, "other")
#Btrueh.GetXaxis().SetBinLabel(2, "charm")
#Btrueh.GetXaxis().SetBinLabel(3, "b")
